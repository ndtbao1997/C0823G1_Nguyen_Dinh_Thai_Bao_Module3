...
<section>
    <div class="container flex" style="width: 50%; height: 100%; padding: 4.5em">
        <form method="post" style="width: 100%; position: relative">
            <h1 style="color: #f44566; text-align: center">Thêm mới chi tiêu</h1>
            <div class="form-group">
                <label class="form-label" for="name">Tên chi tiêu:</label>
                <input type="text" class="form-control" id="name" name="name" placeholder="Tên chi tiêu"
                       value="" pattern="[^\d]+[\p{L}\s]+">
                <div id="emailHelp3" class="form-text" style="color: red">${m1}</div>
            </div>
            <div class="form-group">
                <label class="form-label" for="date">Ngày chi tiêu:</label>
                <input type="date" class="form-control" id="date" name="date" placeholder="YYYY-MM-DD"
                       pattern="^\d{4}/(0[1-9]|1[0-2])/(0[1-9]|[12][0-9]|3[01])">
                <span id="ageError" style="color: red;"></span>
                <div id="emailHelp6" class="form-text" style="color: red">${m3}</div>
            </div>
            <div class="form-group">
                <label class="form-label" for="soTienChi">Nhập số tiền chi:</label>
                <input type="text" class="form-control" id="soTienChi" name="soTienChi" aria-describedby="emailHelp"
                       placeholder="Nhập số tiền chi phải lớn hơn 0"  pattern="^\d{1,10}$">
                <div id="emailHelp5" class="form-text" style="color: red"></div>
            </div>
            <div class="form-group">
                <label class="form-label" for="loaiChi">Loại chi:</label>
                <select name="loaiChi" id="loaiChi">
                    <c:forEach items="${loaiChis}" var="loai">
                        <option value="${loai.maLoaiChi}">${loai.tenLoaiChi}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="form-group">
                <label class="form-label" for="moTa">Mô tả thêm:</label>
                <input type="text" class="form-control" id="moTa" name="moTa" placeholder="Mô tả thêm"
                       value="">
                <div id="emailHelp8" class="form-text" style="color: red">${m2}</div>
            </div>
            <br>
            <br>
            <br>
            <input type="submit" class="btn btn-custom" value="Thêm mới chi tiêu"
